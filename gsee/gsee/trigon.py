...
def aperture_irradiance(direct, diffuse, coords,
                        tilt=0, azimuth=0, tracking=0, albedo=0.06,
                        dni_only=False, angles=None, config={}):
    """
    Args:
    ...
    if config != {} and config['transducer']['solar']['stationary'] == False:
        pitch = config['transducer']['solar']['pitch']
        roll = config['transducer']['solar']['roll']
        incidence = incidence * np.sin(pitch) * np.sin(roll) /(pitch * roll)
        ...
        print(plane_direct, plane_diffuse)
    else:
        plane_direct = (dni * np.cos(incidence)).fillna(0)
        plane_diffuse = (diffuse * ((1 + np.cos(panel_tilt)) / 2)
                         + albedo * (direct + diffuse)
