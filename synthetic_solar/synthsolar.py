...
def eccentricity(n):
    ...
    epsilon = 1.00011 + 0.034221 * np.cos(day_ang) + 0.00128 * np.sin(day_ang) + 0.000719 * np.cos(
        2 * day_ang) + 0.000077 * np.sin(2 * day_ang)
...
def Aguiar_hourly_kt(Kt, n, lat, max_iter):
    ...
    for h in range(1, 25):
        ...
        if (angle_start > -omega) and (angle_end < omega):
            ...
            hs = np.arcsin(
                np.cos(h_ang) * np.cos(delta) * np.cos(np.radians(lat)) + np.sin(delta) * np.sin(np.radians(lat)))
...
def Kt_single_day(Ktm, day):
    """
    Generates a single day synthetic hourly irradiance values G0 (on a horizontal plane)
    based on monthly mean clearness indices. The methods proposed by Aguiar et al for the generation
    of synthetic daily and hourly irradiance values is used to create the sequence.
...
def trend_single_day(lat, day):
    """
    Generates a single day sequence of clear sky (extraterrestrial) hourly irradiance values 
    (on a horizontal plane) based on the annual trend for a given latitude. Estimation of the
    daily irradiation and hourly irradiance on a horizontal plane is based on the method outlined 
    in Chapter 20 of:
    ...
    for d in range(1):
        ...
        # Hourly irradiance on a horizontal plane
        # (Refer to Section 20.5.2 of Luque and Hegedus)
        G0ci = np.pi / 24 * (np.cos(h_ang) - np.cos(omega_s)) / \
               (omega_s * np.cos(omega_s) - np.sin(omega_s)) * (a + b * np.cos(h_ang)) * B0d[d - 1]
        h_sunrise = np.digitize([omega_s], h_start)[0]
        h_sunset = np.digitize([-omega_s], h_end)[0]
        G0ci[0:h_sunrise] = 0
        G0ci[h_sunset:24] = 0
